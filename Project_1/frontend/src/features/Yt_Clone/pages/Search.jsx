      ...
      <div className="p-8 space-y-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex flex-col md:flex-row gap-6 animate-pulse">
            <div className="w-full md:w-80 aspect-video bg-stitch-grey rounded-2xl"></div>
            <div className="flex-1 space-y-4 py-2">
              <div className="h-4 bg-stitch-grey rounded w-3/4"></div>
              <div className="h-3 bg-stitch-grey rounded w-1/2"></div>
              <div className="h-3 bg-stitch-grey rounded w-full"></div>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="max-w-6xl mx-auto p-6 lg:p-10">
      <div className="mb-10 flex items-center gap-4">
        <div className="p-3 bg-stitch-grey rounded-2xl border border-border-main">
          <SearchIcon size={20} className="text-stitch-earth" />
        </div>
        <div>
          <h1 className="text-2xl font-display font-black text-stitch-black tracking-tight">Intelligence Search</h1>
          <p className="text-sm text-text-muted">Showing results for <span className="text-stitch-black font-bold">"{query}"</span></p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {results.length > 0 ? (
          results.map((video) => (
            ...
            >
              <Link to={`/video/${video._id}`} className="w-full md:w-80 shrink-0">
                <div className="relative aspect-video rounded-2xl overflow-hidden bg-stitch-grey border border-border-main">
                  <img 
                    src={video.thumbnail} 
                    alt={video.title} 
                  ...
                  <div className="absolute top-2 left-2 z-10">
                    {video.verification?.finalVerdict === "TRUE" && (
                      <div className="bg-white/90 backdrop-blur-md p-1 rounded-full shadow-sm">
                        <ShieldCheck className="w-3 h-3 text-stitch-green" />
                      </div>
                    )}
                  </div>
                  <div className="absolute bottom-2 right-2 bg-stitch-black text-white text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest">
              ...
              <div className="flex-1 py-1">
                <Link to={`/video/${video._id}`}>
                  <h2 className="text-lg font-display font-bold text-stitch-black leading-tight hover:text-stitch-earth transition-colors line-clamp-2">
                    {video.title}
                  </h2>
                </Link>
                ...
                <div className="mt-4 flex items-center gap-3">
                  <div className="w-6 h-6 rounded-full bg-stitch-grey border border-border-main overflow-hidden shrink-0">
                    {video.channel?.avatar && (
                      <img src={video.channel.avatar} alt="" className="w-full h-full object-cover" />
                    )}
                  </div>
                  <span className="text-[11px] font-bold text-text-muted hover:text-stitch-black transition-colors">
          ...
          <div className="py-20 text-center">
            <SearchIcon size={48} className="mx-auto text-stitch-grey mb-4" />
            <h3 className="text-lg font-display font-black text-stitch-black">No Signals Detected</h3>
