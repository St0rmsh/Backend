    ...
    try {
        for await (const { mode, chunk } of streamGraph(message, threadId)) {
            if (mode === "custom") {
                ...
                send("state", chunk);
                finalState = chunk; // keep overwriting — last one is the complete turn
            }
        }
...
app.get('/threads/:threadId', async (req: Request, res: Response) => {
    try {
        const turns = await getThreadTurns(req.params.threadId);
...
app.patch('/threads/:threadId', async (req: Request, res: Response) => {
    ...
    try {
        const updated = await renameThread(req.params.threadId, title); 
        if (!updated) {
