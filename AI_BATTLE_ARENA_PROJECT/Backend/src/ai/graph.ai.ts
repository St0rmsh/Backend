...
import { createAgent, HumanMessage, AIMessage, providerStrategy, BaseMessage } from "langchain";
...
const write = getWriter() as ((chunk: StreamEvent) => void) | undefined;
    ...
    try {
        ...
        const context = result.answer
            ? `Web search summary: ${result.answer}\n\n` +
        ...
        // Search failing shouldn't fail the whole turn — proceed without
        // web context rather than blocking the user's problem entirely.
...
const solutionNode: GraphNode<typeof state> = async (state) => {
    ...
    // Ground the prompt with search context, without polluting permanent
    // memory (only the plain problem gets saved to messages later).
    const history = state.search_context
        ? [...baseHistory, new HumanMessage(`Relevant current context from the web:\n${state.search_context}`)]
        : baseHistory;
