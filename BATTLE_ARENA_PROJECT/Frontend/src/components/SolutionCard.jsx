...
export const SolutionCard = ({ title, content, score, reasoning, isPrimary }) => {
    ...
    >
      ...
      <motion.div 
        ...
        className={`absolute top-0 right-0 w-40 h-40 -mr-20 -mt-20 rounded-full blur-3xl ${
          isPrimary ? 'bg-(--link-color)' : 'bg-slate-500'
        }`} 
      />

      <div className="flex justify-between items-start relative z-10">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 rounded-2xl glass ${isPrimary ? 'text-(--link-color)' : 'text-slate-400'}`}>
            {isPrimary ? <Trophy size={20} /> : <Star size={20} />}
          </div>
          <div>
      ...
      {/* Score Progress Bar with Entrance Animation */}
      <div className="mt-10 mb-8 relative z-10">
        ...
        <div className="h-3 w-full bg-slate-200/50 dark:bg-slate-800/50 rounded-full overflow-hidden p-1">
           <motion.div 
             ...
             className={`h-1.5 rounded-full shadow-lg ${isPrimary ? 'bg-linear-to-r from-(--link-color) to-(--accent-color)' : 'bg-slate-400'}`}
           />
